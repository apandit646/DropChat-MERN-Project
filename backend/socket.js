...
const socketHandler = (io) => {
  ...
  io.on("connection", async (socket) => {
    ...
    socket.on("sendMessage", async (data) => {
      const { sender, receiver, message, replyId } = data;

      try {
        // If replyId exists, convert it to ObjectId
        const replyObjectId = replyId ? new mongoose.Types.ObjectId(replyId) : null;
        ...
        // Step 2: Populate sender, receiver, and replyId
        const populatedMessage = await Message.findById(newMessage._id)
          .populate("sender", "name email photo")
          .populate("receiver", "name email photo")
          .populate("replyId", "message sender"); // Select fields from the referenced message
