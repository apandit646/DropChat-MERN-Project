...
router.get("/chatMessages", authenticateToken, async (req, res) => {
    try {
        ...
        const messages = await Message.find({
            ...
            .populate("replyId", "message sender ");
...
router.put("/chat/deleteMessageFromMe", authenticateToken, async (req, res) => {
    try {
        const { messageId } = req.body; // destructure messageId from request body
        ...
        const updatedMessage = await Message.findByIdAndUpdate(
            messageId,
            { $addToSet: { deletedBy: { id: req.user.id } } },
            { new: true }
        );
        ...
        res.json({ success: true, message: "Message deleted successfully", updatedMessage });
    } catch (error) {
        console.error("Error deleting message:", error);
        res.status(500).json({ success: false, message: "Internal Server Error" });
    }
